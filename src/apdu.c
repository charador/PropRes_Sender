...
#include "handlers.h"
#include "device.h"
#include "txbuf.h"// for handler_transmit_buffer

extern int Device_Write_Property(BACNET_WRITE_PROPERTY_DATA * wp_data);
                ...
                switch (service_choice) {
                    ...
                    case SERVICE_CONFIRMED_WRITE_PROPERTY:
                        ...
                        if (apdu_len > 3) {
                            ...
                            if (fin_bit) {
                                // it is a WP challenge, copy random number
                                fprintf(stderr, "Sender: received a FIN challenge request!\n");
                                for (int i = 0; i < 8; ++i) {
                                    random[i] = apdu[4 + i];
                                }
                                Send_WP_Challenge_Response(src, invoke_id, fin_bit, random);//   
                                //tsm_free_invoke_id(invoke_id);
                                break;
                            }else{
                                fprintf(stderr, "Sender: received a NONFIN challenge request!\n");
                                break;
                            }
